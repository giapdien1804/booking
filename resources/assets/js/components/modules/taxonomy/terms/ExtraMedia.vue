<template>
    <div class="tags">
        <a class="tag is-link"
           ...
           @click.prevent="openModal(taxonomy)"
           v-text="taxonomy.title"></a>
        <div :class="{'modal':true, 'is-active':isOpen}">
            <div class="modal-background"></div>
            <div class="modal-card column is-10-desktop is-12-tablet">
                <header class="modal-card-head">
                    <p class="modal-card-title" v-text="`${postType.title} - ${modalTitle}`"></p>
                ...
                <section class="modal-card-body">
                    <div class="columns">
                        <div class="column is-3">
                            ...
                            <template v-if="currentTaxTemp ==='default'">
                                <div class="field">
                                    <label>Name</label>
                    ...
                    <div class="field is-grouped">
                        <div class="control" v-if="isSelected && idCheck.length >0">
                            <button class="button is-success" type="button" @click.prevent="sendSelected">
                                Chọn {{modalTitle}} ({{idCheck.length}})
                            </button>
                        </div>
                        <div class="control">
    ...
    export default {
        ...
        mounted() {
            if (_.isObject(this.dataList) || _.isArray(this.dataList))
                this.listTaxonomies = this.dataList;
            else if (this.dataList.length > 0)
                this.listTaxonomies = JSON.parse(this.dataList)

            if (_.isObject(this.DataType) || _.isArray(this.DataType))
                this.postType = this.DataType;
            else if (this.DataType.length > 0)
                this.postType = JSON.parse(this.DataType)
        },
        methods: {
            ...
            doChecked(term, event) {
                let old = _.findIndex(this.checkItem, ['id', term.id]);
                if (old === -1) {
                ...
                this.$nextTick(() => {
                    this.countCheck = _.size(this.checkItem)
                })
            },
            sendSelected() {
            ...
            getTerms() {
                ...
                axios.get(`/content/term/ta/${this.currentTax}`)
                    .then(res => {
            ...
            saveTerm() {
                ...
                if (this.theAction === 1) {
                    axios.post(`/content/term/ta/${this.currentTax}`, data)
                        .then(res => {
                    ...
                    axios.put(`/content/term/ta/${this.currentTax}/te/${this.termForm.id}`, data)
                        .then(res => {
                            if (typeof res.data.message !== 'undefined') {
                                window.Notifi({
                                    'message': res.data.message
            ...
            destroyTerm(term) {
                if (_.findIndex(this.checkItem, ['id', term.id]) !== -1)
                    window.Notifi({
                        title: 'Thông báo',
